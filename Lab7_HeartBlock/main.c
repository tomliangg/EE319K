...
int main(void){
  ...
  while(1){          		// Follows the nine steps list above
    ...
    WaitForASLow(); 		// b) wait for switch to be pressed; recall that the switch on board is negative logic, when it's pressed, it's LOW. when released, it's HIGH
...
void SetVT(void){
// write this function
	GPIO_PORTF_DATA_R |= 0x02;  //set bit 1 so PF1 becomes HIGH
}
...
void ClearVT(void){
// write this function
	GPIO_PORTF_DATA_R &= ~0x02;  //clear bit 1 so PF1 becomes LOW
}
...
void SetReady(void){
// write this function
	GPIO_PORTF_DATA_R |= 0x08;  //set bit 3 so PF3 becomes HIGH
...
void ClearReady(void){
// write this function
	GPIO_PORTF_DATA_R &= ~0x08;  //clear bit 3 so PF3 becomes LOW
}
