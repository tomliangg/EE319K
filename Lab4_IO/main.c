...
int main(void){    
  ...
  while(1){
    ...
    if (SW1&&SW2){                     // both pressed  (is equivalent to both released because of negative logic)
      GPIO_PORTF_DATA_R = 0x00;       // LED is off
    } else{                           
      if (SW1&&(!SW2)){                // just SW1 pressed (is equivalent to only SW2 pressed because of negative logic)
        GPIO_PORTF_DATA_R = 0x08;     // LED is green
      } else{                        
        if ((!SW1)&&SW2){              // just SW2 pressed
          GPIO_PORTF_DATA_R = 0x02;   // LED is red (is equivalent to only SW pressed because of negative logic)
        }else{                        // neither switch (is equivalent to both pressed because of negative logic)
